...
`go-ruby-bcrypt/bcrypt` reimplements Ruby's `bcrypt` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-bcrypt/bcrypt` directly, with no Ruby runtime;
